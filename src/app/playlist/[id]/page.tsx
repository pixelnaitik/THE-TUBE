  ...
  return (
    <div>
      <div className="mb-8 mt-2 flex flex-col gap-6 md:flex-row">
        <aside className="glass-panel group relative flex min-h-[300px] w-full shrink-0 flex-col justify-end overflow-hidden rounded-2xl p-6 md:w-80">
          {validVideos.length > 0 && validVideos[0].video.thumbnail && (
            <div className="absolute inset-0 z-0 opacity-40 transition-opacity group-hover:opacity-50">
              <Image src={validVideos[0].video.thumbnail} alt="" fill className="scale-110 object-cover blur-md" sizes="320px" />
            </div>
          )}
          <div className="absolute inset-0 z-10 bg-gradient-to-t from-black/90 via-black/60 to-transparent" />

          <div className="relative z-20">
            <h1 className="page-title mb-2 leading-tight text-white">{playlist.name}</h1>
          ...
          {validVideos.length > 0 ? (
            <div className="flex flex-col gap-3">
              {validVideos.map((item, index) => (
                <div key={item.id} className="glass-panel group flex items-start gap-4 rounded-xl p-2 transition-colors hover:bg-[var(--surface-2)]/60">
                  ...
                  <div className="min-w-0 flex-1 py-1">
                    <Link href={`/watch/${item.video.id}`} className="mb-1 line-clamp-2 text-base font-semibold text-[var(--foreground)] transition-colors group-hover:text-blue-300 md:text-lg">
                      {item.video.title}
                    </Link>
